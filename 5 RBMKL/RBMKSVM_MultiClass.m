function Res = RBMKSVM_MultiClass(trainSet, testSet, conf)
%
% train = {c1, c2, ..., cn}
% c = [X, label]
% test =  [X, label]
...
for i = 1 : totalClass
    ...
    for j = i +1 : totalClass
        ...
        [temp, t]= RBMKSVM_OAO(classOne , classTwo , testSet(:,1:dim-1), conf) ;
        t_train = t_train + t ;
        forSave{i,j} = temp;
...
function [forSave, t] = RBMKSVM_OAO(classOne, classTwo, test, conf)
%
...
%
    ...
    output = rbmksvm_test(testing_data, model);
